...
void Gen9_5::init() {
    callback = this;
	if (NBlue::callback->deviceId == 0x3E90 || NBlue::callback->deviceId == 0x3E93 || NBlue::callback->deviceId == 0x3E99 || NBlue::callback->deviceId == 0x3E9C || NBlue::callback->deviceId == 0x3EA1 || NBlue::callback->deviceId == 0x9BA5 || NBlue::callback->deviceId == 0x9BA8 || NBlue::callback->deviceId == 0x3185 || NBlue::callback->deviceId == 0x3184) {
...
bool Gen9_5::processKext(KernelPatcher &patcher, size_t index, mach_vm_address_t address, size_t size) {
    if (kextG9_5FB.loadIndex == index) {
		...
		NBlue::callback->patchset.MiscNames->fb = NBlue::callback->isCflDerivative ? "AppleIntelCFLGraphicsFramebuffer" : "AppleIntelKBLGraphicsFramebuffer";
